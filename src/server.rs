...
struct AppConfig {
    ...
    config_path: PathBuf,
    source: DbPathSource,
    configured_db_path: Option<String>,
...
#[derive(Clone, Debug)]
struct DbStartupBackup {
    created_at: String,
    backup_path: PathBuf,
    reason: String,
    from_app_id: Option<String>,
    from_app_version: Option<String>,
    from_schema_version: Option<i64>,
    from_proto_version: Option<i64>,
}
...
async fn prepare_db_startup_backup(
    db_path: &StdPath,
...
#[derive(Clone)]
struct SecretStorageInfo {
    storage_version: String,
    migrated_at: String,
    configured_secrets: usize,
    encrypted_secrets: usize,
    key_source: String,
    key_location: String,
    key_status: String,
...
fn is_encrypted_secret_value(value: &str) -> bool {
...
fn secret_storage_error(message: impl Into<String>) -> sqlx::Error {
...
fn encrypt_secret_value(setting_key: &str, plaintext: &str) -> Result<String, String> {
...
fn decrypt_secret_value(setting_key: &str, stored: &str) -> Result<String, String> {
...
fn inspect_secret_key_runtime() -> (String, String, String) {
    if let Ok(raw) = env::var(SECRET_MASTER_KEY_ENV) {
        let trimmed = raw.trim();
        if !trimmed.is_empty() {
...
fn write_db_path_config(path: &StdPath, value: &str) -> std::io::Result<()> {
    if let Some(parent) = path.parent() {
        if !parent.as_os_str().is_empty() {
            fs::create_dir_all(parent)?;
        }
...
async fn update_db_path(Form(form): Form<UpdateDbPathForm>) -> AppResult<Redirect> {
    ...
    if trimmed.is_empty() {
        ...
        write_db_path_config(&config_path, &resolved).map_err(internal_error)?;
    }
